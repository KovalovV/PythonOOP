...
from json_validate import Validate_JSON

from const_course import COURSES_JSON, TEACHERS_JSON, SCHEMA_COURSE, SCHEMA_TEACHER
...
class ITeacher(ABC):
    ...
    data_of_teachers = Validate_JSON(SCHEMA_TEACHER, TEACHERS_JSON).validate()
    ...
    def __str__(self):
        ...
        courses = ', '.join(
            [f'{array_of_courses[i]}' for i in range(len(array_of_courses))])
    ...
    def name(self, name):
        ...
        if not Teacher.data_of_teachers.get(f'{name} {self.surname}'):
...
class ICourse(ABC):
    ...
    data_of_courses = Validate_JSON(SCHEMA_COURSE, COURSES_JSON).validate()
    ...
    def __str__(self):
        ...
        theme = ', '.join(
            [f'{item_course_theme[i]}' for i in range(len(item_course_theme))])
        teachers = ', '.join(
...
class CourseFactory(ICourseFactory):
    ...
    def create_item(self, creating_obj, arg1, arg2):
        if not isinstance(arg1, str) or not isinstance(arg2, (str, list)):
            raise TypeError(
                'Wrong field type, waiting for a string or Teacher to second arg')
...
def main():
    ...
    # with open(TEACHERS_JSON) as teachers:
    #     data_of_teachers = json.load(teachers)

    # jsonschema.validate(data_of_courses, schema_course)

    # try:
    #     jsonschema.validate(data_of_courses, schema_course)
    #     jsonschema.validate(data_of_teachers, schema_teacher)
    ...
    course_local = CourseFactory().create_item(
        'local course', 'Python 2022 3.0', [teacher_local, teacher_local1])
    print(course_local)

    teacher_offsite = CourseFactory().create_item('teacher', 'Vadym', 'Shpuryk')
    print(teacher_offsite)

    course_offsite = CourseFactory().create_item(
        'offsite course', 'C++ 2022 13.0', [teacher_offsite])
