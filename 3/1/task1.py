...
from constants_for_task1 import LATE, REGULAR, ADVANCE, STUDENT
...
class Ticket():
    ...
    with open('C:/MinGW/bin/cpp/Python/3/tickets.json') as tickets:
    ...
    def update_data(self, name_event, type_ticket):
        ...
        with open('C:/MinGW/bin/cpp/Python/3/tickets.json', 'w') as update_tickets:
            update_tickets.write(json.dumps(Ticket.data_of_tickets, indent=4))

    def sold_data(self, name_event, type_ticket):
        with open('C:/MinGW/bin/cpp/Python/3/sold_tickets.json', 'w') as sold_tickets:
...
class Order:
    """
    Order class. Gets customer information and additional pizza ingredients.
    """

    def __init__(self, customer, ticket):
        if not isinstance(customer, Customer) or not isinstance(ticket, Ticket):
            raise TypeError(
                'Uncorrect type of input data (Customer or Ticket)')
...
it_event1 = Regular('Python Developer CyberBionic Systematics')
it_event2 = Late('Blockchain & Bitcoin Conference Kyiv 2021')
it_event3 = Student('Salesforce Meetup')
it_event4 = Regular('Salesforce Meetup')

customer1 = Customer('Vitala', 'Kovalov', '0978557148', True)
customer2 = Customer('Vlad', 'Sergiuchuk', '0954813729')
