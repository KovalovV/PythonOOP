...
class Pizza:
    """
    Super class Pizza, from which all kinds of other pizzas are inherited.
    """
...
"""
7 types of "Pizza Day", which are inherited from the super class Pizza.
"""
...
class Order:
    ...
    def count_price(self):
        self.__order_price = self.__pizza.price
        for ingredient in self.__ingredients:
            self.__pizza.ingredients.append(ingredient)
            self.__order_price += Order.data_of_ingradients[ingredient]['price']
        return self.__order_price


def main():
    customer1 = Customer('Vitala', 'Kovalov', '0978557148')
    order1 = Order(customer1, ['bananas'])
    print(order1)
